...
const queueController = {
    ...
    getAvailablePatients: async (req, res) => {
        try {
            const rawResult = await query(`SELECT * FROM patients ORDER BY first_name ASC`);
    ...
    updateQueueStatus: async (req, res) => {
        try {
            ...
            // ✅ UPDATE: Removed 'RECEPTION' from this restricted list
            const restrictedRoles = ['NURSE', 'STUDENT'];
            if (status === 'Treatments are done / Done' && restrictedRoles.includes(userRole)) {
                return res.status(403).json({
                    success: false,
                    message: `Your role (${userRole}) cannot mark treatments as done.`
                });
            }

            await query('UPDATE queue SET status = ? WHERE id = ?', [status, queueId]);
